    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::UserServersDisabled => {
                write!(f, "user-managed MCP servers are disabled by policy")
            }
...
impl Default for McpPolicyAction {
    fn default() -> Self {
        Self::Allow
    }
}
...
impl ManagedMcpSettings {
    fn from_root(root: &TomlValue) -> std::io::Result<Self> {
        ...
        if let Some(enabled) = section.enable_mcp_servers {
            settings.flags.managed_enabled = enabled;
        }
...
fn resolve_mcp_servers_internal(
    ...
    if !resolution.flags.managed_enabled
        && (!managed_config_entries.is_empty() || !managed_preferences_entries.is_empty())
    {
        return Err(std::io::Error::new(
            ErrorKind::PermissionDenied,
            "managed MCP servers present but disabled via managed.enable_mcp_servers = false",
        ));
    }
...
pub async fn load_mcp_server_resolution(codex_home: &Path) -> std::io::Result<McpServerResolution> {
...
pub async fn load_global_mcp_servers(
    ...
    let resolution = load_mcp_server_resolution(codex_home).await?;
...
"#,
    ...
    #[tokio::test]
    async fn managed_mcp_servers_disabled_fails_if_present() -> anyhow::Result<()> {
        let codex_home = TempDir::new()?;
        let managed_path = codex_home.path().join("managed_config.toml");
...
"#,
        ...
        let layers = load_config_layers_with_overrides(codex_home.path(), overrides).await?;
        let err = resolve_mcp_servers_for_layers(
            layers.base,
            Vec::new(),
            layers.managed_config,
            layers.managed_preferences,
        ...
        .expect_err("managed entries should fail when disabled");
